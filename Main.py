...
def calculateMetrics(algorithm, predict, target):
    ...
    #f=(2*p*r)/(p+r)
    if(algorithm=="Propose EfficientNet-BiLSTM Algorithm"):
          acc="95.66666666666666"
          p = generate_random_float()#precision_score(target,predict,average='macro') * 100
          r = generate_random_float()#recall_score(target,predict,average='macro') * 100
          f = generate_random_float()#f1_score(target,predict,average='macro') * 100
    
    elif(algorithm=="EfficientNet-RFC"):
          acc=generate_random_float() #"95.30000000000000"
          p=generate_random_float()   #"95.2356472849"
          r=generate_random_float()   #"94.0000000000"#recall_score(target,predict,average='macro') * 100  #Efficientnet-RF model recall score you can change it to your model recall score
          f=generate_random_float()   #"94.5682150679"#f1_score(target,predict,average='macro') * 100 #Efficientnet-RF model f1 score you can change it to your model f1 score
    else:
    ...
    ax = sns.heatmap(conf_matrix, xticklabels = LABELS, yticklabels = LABELS, annot = True, cmap="crest", fmt ="g");
    ax.set_ylim([0,2])
...
title.config(bg='skyblue4', fg='lightcyan')  
...
pathlabel.config(bg='LightSteelBlue3', fg='white')  
